...
SpinBox::SpinBox(VECTOR2 pos,int xSize,BoxType type, int fontHandle)
{
	...
	selKey_ = 0;
	nowSize_ = 0;
	button_.first = std::make_unique<ImageRectButton>(pos+size_ - VECTOR2(24+2,size_.y-2),VECTOR2(24,12), L"./data/image/allow_up.png", L"./data/image/allow_up_push.png", [&]() {selKey_ -= (selKey_ - 1 < 0 ? 0 : 1);  return true; }, VECTOR2());
	button_.second = std::make_unique<ImageRectButton>(pos+size_ - VECTOR2(24+2,12+2), VECTOR2(24, 12), L"./data/image/allow_down.png", L"./data/image/allow_down_push.png", [&]() {selKey_ += (selKey_ + 1>= nowSize_ ? 0 : 1);  return true; }, VECTOR2());
...
SpinBox::SpinBox(VECTOR2 pos, VECTOR2 size, BoxType type)
{
	pos_ = pos;
	type_ = type;
	size_ = size;
