...
PullDown::PullDown(VECTOR2 pos, int xSize,int fontHandle)
{
	...
	selKey_ = 0;
	list_.emplace_back(std::make_unique<RectButton>( pos_+VECTOR2(size_.x-fontSize, 0), VECTOR2(fontSize,fontSize), 0xfffffff, [&]() {return openFlag_ = ~openFlag_; }));
...
bool PullDown::Add(std::string str)
{
	...
	const int fontSize = GetFontSizeToHandle(fontHandle_);
	list_.emplace_back(std::make_unique<RectButton>(VECTOR2(0, listSize * fontSize), VECTOR2(size_.x , fontSize ), 0xffffff, [&, listSize]() {selKey_ = listSize; openFlag_ = false; return true; }));
