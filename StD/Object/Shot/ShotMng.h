...
public:
	...
	void Draw(void);				//弾の描画
	void AddBullet(std::shared_ptr<Unit> ptr, std::shared_ptr<Unit> target/*目標のポインタ*/, float deltaTime);			//Unitから発射される弾の作成
	std::shared_ptr<Unit> BulletMove(std::shared_ptr<Unit> ptr/*射出点ユニット*/, std::shared_ptr<Unit> target/*射出点座標*/, float deltaTime);	//弾の移動
	void BulletControler(float deltaTime, std::shared_ptr<PlayerMng> playerMng, std::shared_ptr<EnemyManager> enemyMng);
	...
	bool isCoolTime(std::shared_ptr<Unit> ptr, float deltaTime);
